...
from source_code_task2.utils import (
  timestamp,
  show_training_result,
  load_data,
  add_technical_features
)
from source_code_task2.methods import train_model, evaluate_model
from source_code_task2.agent import RLAgent
import pandas as pd
...
def run(training_stock, validation_stock, window_size, batch_size, episode_count, model_type="ddqn", model_name = None, pretrained = False, verbose = False):
  training_data = add_technical_features(load_data(
      training_stock), window=window_size).sort_values(by=['Date'], ascending=True)
  validation_data = add_technical_features(load_data(
      validation_stock), window=window_size).sort_values(by=['Date'], ascending=True)
