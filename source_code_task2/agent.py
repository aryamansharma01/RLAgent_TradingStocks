...
from source_code_task2.utils import timestamp
import math
import pdb
...
class RLAgent:
  ...
  def replay(self, batch_size):
    ...
    if self.model_type == 'ddqn':
      ...
      for state, time, action, reward, next_state, next_time, done in mini_batch:
        ...
        if done:
          ...
          target = reward + gamma * self.target_model.predict(next_state)[0][np.argmax(self.model.predict(next_state)[0])]
  ...
  def remember(self, state, time, action, reward, next_state, next_time, done):
    self.memory.append((state, time, action, reward, next_state, next_time, done))
